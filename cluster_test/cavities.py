...
from numpy import log10,sqrt,trapz,heaviside,gradient,pi,linspace,array,outer,fft,arange,zeros,empty,real,imag,complex128,sum,exp,sin,cos,dot
import numpy as np
...
def freq_appoach(TFhp,omega,fmin,fmax,cavity,Rmax,K=5,l=1,B0=5):
    ...
    for k in arange(len(alphak)):
        coef.append(((alphak[k]*(c/l))**2-omega**2))
    Ak = array(-sum(real(TFsk/coef),axis=1))
    Bk = array(sum(omega*imag(TFsk/coef),axis=1)/(alphak*c/l))
    dirack=[]
    t1=linspace(0,1/deltaF,2*len(omega)-2)
    for k in range (0,len(alphak)):
        a = Ak[k]*fft.rfft(cos((alphak[k]*(c/l-deltaF))*t1))*2/len(t1) + Bk[k]*fft.rfft(sin(alphak[k]*(c/l-deltaF)*t1))*2/len(t1)
        dirack.append(a)

    TFbk = TFsk/coef #+ array(dirack)

    for k in range (len(alphak)):
            if 2*pi*fmin < alphak[k]*c/l < 2*pi*fmax:
                TFbk[k] = TFbk[k] + array(dirack[k])


    
    #for k in range(len(alphak)):
    #    if -log(M,10)< 4.8 and alphak[k]*c/l < 2*pi*fmax:
    #        TFbk[k] = despike(TFbk[k],10**-29*10**(-log(M,10)-4)*(k+1)**-2)
    # if MS > 5.2 and alphak[-1]*c/l/2/pi > f_min:
    #     for k in range(len(alphak)):
    #         TFbk[k] = despike(TFbk[k],10**-27*10**(6-MS)*(k+1)**-2)

    TFdE_f = 4*pi*B0/mu0*dot(Ik,TFbk)#*heaviside(-omega/2/pi+fisco,0.5)*heaviside(omega/2/pi-f_min,0.5)
    #TFdP_f = 1j*omega*TFdE_f
    dE_f = fft.irfft(TFdE_f)*len(TFdE_f)
    tdE_f = linspace(0,1/deltaF,len(dE_f))

    # harmodP=0
    # for k in range (0,len(alphak)):
    #     a = -alphak[k]*(c/l-deltaF)*Ak[k]*sin(alphak[k]*(c/l-deltaF)*tdE_f) + alphak[k]*(c/l-deltaF)*Bk[k]*cos(alphak[k]*(c/l-deltaF)*tdE_f)
    #     harmodP+=2*pi*B0/mu0*a*Ik[k]


    dP_f = gradient(dE_f,tdE_f[1]-tdE_f[0],edge_order=2)#+ 2*10**(-11)*sin(alphak[0]*c/l*t1+0.55) #+ harmodP
    #dP_f = fft.irfft(TFdP_f)*len(TFdP_f)
    TFdP_f = fft.rfft(dP_f)*2/len(dP_f)#*(heaviside(-omega/2/pi+fisco,0.5)+0.005)#*(heaviside(omega/2/pi-f_min,0.5)+0.1)
    rmsP_f = sqrt(trapz(dP_f**2,tdE_f)/max(tdE_f))
    #print('Frequency approach done in (s)')
    #print(time()-t0)
    return rmsP_f,TFdP_f,dP_f,tdE_f
...
def time_approach(hp,t,cavity,Rmax,K=5,l=1,B0=5):
    ...
    sol=zeros((K,len(T)))
    temp=empty((K,len(T)),dtype=complex128)
    print(len(freq))
    for k in arange(len(freq)):
        if k%100==0:
            print(k)
        outer(transfo[:,k],exp(1j*freq[k]*T),temp)
        sol+=real(temp)
    #temp=einsum('i,j->ij',freq,T)
    #sol=einsum('ij,jk->ik',transfo,exp(1j*temp))
    # def test(k):
    #     outer(transfo[:,k],exp(1j*freq[k]*T),temp)
    #     sol+=real(temp)
    # pool = Pool(4)
    # pool.map(test, arange(len(freq)))
    # pool.close()

    phase=outer(alphak,T)
    A=array(-sum(real(transfo),axis=1))
    B=array(sum(imag(transfo)*freq,axis=1)/alphak)
    for k in arange(len(alphak)):
        sol[k,:]+=A[k]*cos(phase[k,:])+B[k]*sin(phase[k,:])

    dE_t=(4*pi*B0**2/mu0*hGW)*dot(Ik,sol)
    ...
    print(time()-t0)
    print(trapz(dE_t,tdE_t)/tmax)
    print(trapz(dP_t,tdE_t)/tmax)
    print(sqrt(trapz(dE_t**2,tdE_t)/tmax))
    return rmsP_t,dP_t
...
def fct_bode(fspan,cavity,Rmax,K=5,l=1,B0=5):
    ...
    testan=0
    for k in arange(len(alphak)):
        testan+=Ik[k]**2/normk[k]/abs((4*pi**2*fspan**2-alphak[k]**2*c**2/l**2))
    return c*4*pi**2*B0**2*l**2*4*pi**2*fspan**2/mu0*abs(sin(pi*fspan*l/c))*testan

def RMS_mass_deno(M,d,cavity,Rmax):
    fisco_arr = 2200/M
    fmin_arr = fisco_arr/d
    ftest=10**linspace(log10(fmin_arr),log10(fisco_arr),100000)
    A=sqrt(5/24)/pi**(2/3)
    chirpmass=M*MSUN_SI**(6/5)/(2*M*MSUN_SI)**(1/5)
    dist=1e9*PC_SI
    rep=2*(ftest[1]-ftest[0])*A*c/dist*(G*chirpmass/c**3)**(5/6)/(2*pi*ftest)**(7/6)
    a = fct_bode(ftest,cavity,Rmax)*rep
    RMS=sqrt(sum(a**2))
    return RMS
...
def Sh_infl(f):
    return (hc_infl**2)*(f/fcut1)**(-alpha_infl)

def Sh1_pt(f):
    return (hc_pt**2)*(f/fcut2)**(-alpha1_pt)
def Sh2_pt(f):
    return (hc_pt**2)*(f/fcut2)**(-alpha2_pt)
...
def get_sgwbs(freq):
    TFhp_infl = sqrt(Sh_infl(freq))*(heaviside(-freq+fcut1,0.5))
    TFhp_pt = []
    for f in freq :
        if f <= fcut2:
