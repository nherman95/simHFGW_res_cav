...
def cav_parameters(cavity,K,l,Rmax,Rmin=0.1):
    if(cavity=='TM'):
        ...
        Ikr=0.5*(2*alpha_k*Rmax-pi*alpha_k*Rmax*struve(1,alpha_k*Rmax))*jv(0,alpha_k*Rmax)
        Ikth=(jv(0,alpha_k*Rmax)-1)/alpha_k
        normkr=(1-jv(0,alpha_k*Rmax)**2)/2
        normkth=(-1+(alpha_k**2*Rmax**2*+1)*jv(0,alpha_k*Rmax)**2)*0.5
    elif(cavity=='TEM'):
    ...
    return alpha_k,Ikr,Ikth,normkr,normkth
...
def time_approach(hp,t,cavity,Rmax,K=5,l=1,B0=5):
    t0 = time()
    alphak,Ikr,Ikth,normkr,normkth=cav_parameters(cavity,K,l,Rmax)
    T=c/l*t ; dt=t[1]-t[0] ; dT=T[1]-T[0] ; tmax=t[-1]
    hGW=max(abs(hp)) ; hp=hp/hGW
    hinter=CubicSpline(T,hp); nbrzeros=200
    TD=T-1; TD[TD<0]=0 ; scoefr=pi*Ikr/normkr ; scoefth=pi*Ikth/normkth
    sint=hinter(T,1)-hinter(TD,1)
    skr=outer(scoefr,sint)
    skth=outer(scoefth,sint)
    ...
    f = c/l*fft.rfftfreq(len(skr[0])+200,dT)
    transfor=2./(len(T)+nbrzeros)*fft.rfft(skr,axis=1,n=len(T)+nbrzeros)/coef
    transfoth=2./(len(T)+nbrzeros)*fft.rfft(skth,axis=1,n=len(T)+nbrzeros)/coef
    ...
    harmosc_solver.solver(T,alphak,freq,real(transfor),imag(transfor),Q)
    
    dE_t=(-7/6*pi*B0**2/mu0*hGW)*dot(Ikr,harmosc_solver.sol)
    harmosc_solver.sol=None
    harmosc_solver.solver(T,alphak,freq,real(transfoth),imag(transfoth),Q)
    dE_t=dE_t+(-7/6*pi*B0**2/mu0*hGW)*dot(Ikth,harmosc_solver.sol)
    harmosc_solver.sol=None
...
def freq_appoach(TFhp,omega,cavity,Rmax,K=5,l=1,B0=5):
    alphak,Ikr,Ikth,normkr,normkth=cav_parameters(cavity,K,l,Rmax)
    deltaF=1/2/pi*(omega[1]-omega[0])
    TFskr = 7/3*pi*B0*c*outer(Ikr/normkr,omega*TFhp*sin(omega*l/2/c))
    TFskth = 7/3*pi*B0*c*outer(Ikth/normkth,omega*TFhp*sin(omega*l/2/c))
    coef=[]
    for k in arange(len(alphak)):
        coef.append(((alphak[k]*(c/l))**2-omega**2+1j*omega*alphak[k]/Q*c))
    Akr = array(-sum(real(TFskr/coef),axis=1))
    Bkr = array((-alphak/2/Q*c*Akr+sum(omega*imag(TFskr/coef),axis=1))/(alphak*c/l)/sqrt(1-1/2/Q))
    dirackr=[]
    Akth = array(-sum(real(TFskth/coef),axis=1))
    Bkth = array((-alphak/2/Q*c*l*Akth+sum(omega*imag(TFskth/coef),axis=1))/(alphak*c/l)/sqrt(1-1/2/Q))
    dirackth=[]
    for k in range (0,len(alphak)):
        a=Akr[k]/(-alphak[k]/2/Q*c*l+1j*(omega-alphak[k]*c/l*sqrt(1-1/2/Q)))+1j*Bkr[k]/(-alphak[k]/2/Q*c*l+1j*(omega-alphak[k]*c/l*sqrt(1-1/2/Q)))
        dirackr.append(a)
        a=Akth[k]/(-alphak[k]/2/Q*c*l+1j*(omega-alphak[k]*c/l*sqrt(1-1/2/Q)))+1j*Bkth[k]/(-alphak[k]/2/Q*c*l+1j*(omega-alphak[k]*c/l*sqrt(1-1/2/Q)))
        dirackth.append(a)
    TFbkr = TFskr/coef + array(dirackr)
    TFbkth = TFskth/coef + array(dirackth)
    TFdE_f_p = pi*B0/mu0*(dot(Ikr,TFbkr)+dot(Ikth,TFbkth))#*heaviside(-omega/2/pi+fisco,0.5)*heaviside(omega/2/pi-f_min,0.5)
    TFdP_f_p = 1j*omega*TFdE_f_p
    rmsP_f = sqrt(sum(abs(TFdP_f_p)**2)/len(TFdP_f_p))
    dP_f = fft.irfft(1j*omega*pi*B0/mu0*(dot(Ikr,TFskr/coef)+dot(Ikth,TFskth/coef)))*len(TFdP_f_p)
    tdE_f = linspace(0,1/deltaF,len(dP_f))
    freq=alphak*c*sqrt(1-1/2/Q)
    for i in range(len(tdE_f)):
        dP_f[i]=dP_f[i]+(dot((Akr+Akth)*exp(-alphak/2/Q*c*tdE_f[i]),-freq*sin(freq*tdE_f[i]))+dot((Bkr+Bkth)*exp(-alphak/2/Q*c*tdE_f[i]),freq*cos(freq*tdE_f[i])))*pi*B0/mu0-(dot(alphak/2/Q*c*exp(-alphak/2/Q*c*tdE_f[i])*(Akr+Akth),cos(freq*tdE_f[i]))+dot(alphak/2/Q*c*exp(-alphak/2/Q*c*tdE_f[i])*(Bkr+Bkth),sin(freq*tdE_f[i])))*pi*B0/mu0
    return rmsP_f,TFdP_f_p,dP_f,tdE_f

def fct_bode(fspan,cavity,Rmax,K=10,l=1,B0=5):
    alphak,Ikr,Ikth,normkr,normkth=cav_parameters(cavity,K,l,Rmax)
    testan=0
    for k in arange(len(alphak)):
        testan+=(Ikr[k]**2/normkr[k]+Ikth[k]**2/normkth[k])/sqrt((4*pi**2*fspan**2-alphak[k]**2*c**2/l**2)**2 +(alphak[k]*2*pi*fspan*c/2/Q)**2)
